...
import { escapeCSV, downloadBlob } from "./utils";
...
function downloadCSV(csvContent: string, filename: string): void {
  const BOM = '\uFEFF';
  downloadBlob(new Blob([BOM + csvContent], { type: 'text/csv;charset=utf-8;' }), filename);
}
...
export function exportWellsToCSV(features: Feature[]): void {
  ...
  const dataRows = features.map(feature => {
    ...
    return WELL_ATTRIBUTES.map(attr => escapeCSV(properties[attr.key])).join(';');
  });
...
export function exportFeaturesToCSV(features: Feature[], filename: string): void {
  ...
  const dataRows = features.map(feature => {
    const properties = feature.getProperties();
    return keys.map(key => escapeCSV(properties[key])).join(';');
  });
